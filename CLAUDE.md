...
- **Confirm instance-specific values against the running v20 app** before relying on them.
  ...
  - Error leakage (A05): `GET /rest/products/search?q=')` → **500** with `Content-Type: text/html` (a raw Express error page, not the JSON envelope) whose `<title>`/`<h2>` carry `SQLITE_ERROR: near ")": syntax error` plus the banner `OWASP Juice Shop (Express ^4.22.1)`. **A lone single quote `'` does NOT leak** — it returns `200 {"status":"success","data":[]}`; the doc's `q: "'"` is wrong for v20 (use `')`, which closes the LIKE literal and unbalances the WHERE parens). The full `SELECT ... FROM Products` query text is **not** echoed — only the engine error — so assert on the engine token `/SQLITE_ERROR/i`, not a query fragment.
